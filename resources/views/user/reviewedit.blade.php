    ...
    <div class="min-h-screen bg-gray-50 py-10">
        <div class="max-w-4xl mx-auto">
            <div class="bg-white rounded-2xl shadow-lg p-8">
                <div class="mb-4">
                    <a href="{{ route('orders.index', ['status' => 'delivered']) }}" class="text-gray-700 hover:text-gray-900 font-medium">
                        &larr; Kembali ke Pesanan
                    </a>
                </div>
                <h1 class="text-3xl font-bold text-gray-900 mb-2">Edit Ulasan</h1>
                ...
                <div class="mt-8 border-t pt-6">
                    ...
                    <div class="flex items-center space-x-6">
                        <div class="w-28 h-28 bg-gray-100 rounded-xl overflow-hidden flex-shrink-0">
                            @if ($review->product->images->count() > 0)
                                <img src="{{ asset('storage/' . $review->product->images->first()->image_path) }}"
                                    alt="{{ $review->product->product_name }}" class="w-full h-full object-cover">
                            @else
                        ...
                        <div class="flex-1">
                            <h3 class="text-xl font-semibold text-gray-900">{{ $review->product->product_name }}</h3>
                        </div>
                    </div>
                </div>
                ...
                <form action="{{ route('reviews.update', $review->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <input type="hidden" name="transaction_id" value="{{ $review->transaction_id }}">
                    ...
                    <div class="border-t pt-6">
                        ...
                        <textarea name="comment" id="comment" rows="5"
                            ...
                            placeholder="Bagikan pengalaman Anda dengan produk ini...">{{ old('comment', $review->comment) }}</textarea>
                        @error('comment')
                            <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                        @enderror
                    </div>

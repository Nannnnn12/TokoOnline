...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="bg-white rounded-2xl shadow-lg p-8">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-2">
                    ...
                    <div class="flex flex-wrap gap-1 sm:gap-1">
                        <a href="{{ route('products.show', $product) }}?sort=all"
                            class="filter-btn px-2 sm:px-3 py-1 text-xs rounded-full {{ $sortBy == 'all' ? 'bg-yellow-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                            Semua
                        </a>

                        @for ($i = 5; $i >= 1; $i--)
                            <a href="{{ route('products.show', $product) }}?sort=rating_{{ $i }}"
                                class="filter-btn px-2 sm:px-3 py-1 text-xs rounded-full {{ $sortBy == 'rating_' . $i ? 'bg-yellow-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }} flex items-center space-x-1">
                                <span>{{ $i }}</span>
                        ...
                        <a href="{{ route('products.show', $product) }}?sort=date_desc"
                            class="filter-btn px-2 sm:px-3 py-1 text-xs rounded-full {{ $sortBy == 'date_desc' ? 'bg-yellow-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                            Terbaru
                        </a>
                    </div>
                </div>
        ...
        <div class="space-y-6">
            ...
            @php $showLimit = 5; @endphp

            @if ($product->reviews->count() > 0)
                @foreach ($product->reviews as $index => $review)
                    <div class="review-item {{ $index >= $showLimit ? 'hidden' : '' }}">
                ...
                @if ($product->reviews->count() > $showLimit)
                    <div class="text-center mt-6 space-x-2">

                        <!-- Tombol Lihat Selengkapnya -->
                        <button id="showMoreBtn"
                            class="px-4 py-2 bg-gray-200/70 hover:bg-gray-300 text-gray-700 text-sm rounded-lg transition">
                            Lihat Selengkapnya
                        </button>

                        <!-- Tombol Tutup -->
                        <button id="showLessBtn"
                            class="px-4 py-2 bg-gray-200/70 hover:bg-gray-300 text-gray-700 text-sm rounded-lg transition hidden">
                            Tutup
                        </button>
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // Jika tidak ada tombol, keluar
        if (!btnMore || !btnLess) return;

        // Buka (lihat selengkapnya)
